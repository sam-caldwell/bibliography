...
func TestFetchSong_FromITunes(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("FetchSong: %v", err)
	}
	if e.Type != "song" || e.APA7.Title == "" || e.APA7.URL == "" {
		t.Fatalf("entry not populated: %+v", e)
	}
	if _, err := url.Parse(e.APA7.URL); err != nil {
...
func TestFetchSong_MusicBrainzFallback(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("MusicBrainz fallback: %v", err)
	}
	if e.APA7.Title == "" || e.APA7.ContainerTitle == "" {
		t.Fatalf("expected fields from MB: %+v", e)
	}
}
