...
func UpdateBibEntry(e schema.Entry) error {
	...
	for i := range records {
		r := &records[i]
		if strings.TrimSpace(r.fields["created"]) == "" {
			r.fields["created"] = now
		}
		...
		if vstr == "true" {
			if strings.TrimSpace(r.fields["verified_by"]) == "" {
				r.fields["verified_by"] = gitUserName()
			}
		} else {
		...
		if strings.ToLower(strings.TrimSpace(r.fields["_id"])) == idLower {
			...
			if strings.TrimSpace(r.fields["modified"]) == "" {
				r.fields["modified"] = now
			}
		} else {
			if strings.TrimSpace(r.fields["modified"]) == "" {
			...
			if strings.TrimSpace(r.fields["source"]) == "" {
				r.fields["source"] = "manual"
			}
		}
	}
	// Deterministic order
