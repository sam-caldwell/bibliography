...
func BuildKeywordIndex(entries []schema.Entry) (string, error) {
	...
	for _, e := range entries {
		...
		// 2) words in the title (and implicitly names within title)
		for _, w := range tokenizeWords(e.APA7.Title) {
			add(w)
		}

		// 3) publisher (full phrase and tokens)
		...
		// 4) publication container/journal (full phrases and tokens)
		...
		// 6) website domain (host and host without leading www.)
		...
		// 7) type (e.g., article, book, website)
...
func MigrateExisting() ([2][]string, error) {
	var from []string
	var to []string
	// Ensure base dir exists
	if _, err := os.Stat(CitationsDir); errors.Is(err, fs.ErrNotExist) {
		return [2][]string{from, to}, nil
	}
	entries, err := os.ReadDir(CitationsDir)
	if err != nil {
		return [2][]string{}, err
	}
	for _, d := range entries {
		...
		oldPath := filepath.Join(CitationsDir, d.Name())
		// Load YAML to get type/id
		...
		var e schema.Entry
		if err := yaml.Unmarshal(data, &e); err != nil {
			// Skip invalid YAML rather than failing the whole migration
			continue
		}
		...
		newPath := filepath.Join(subdir, fmt.Sprintf("%s.yaml", e.ID))
		if oldPath == newPath {
			continue
		}
		// If target exists, skip to avoid overwrite
		...
		from = append(from, oldPath)
		to = append(to, newPath)
	}
	return [2][]string{from, to}, nil
}
