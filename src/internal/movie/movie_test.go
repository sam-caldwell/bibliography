...
func TestFetchMovie_FromOMDb(t *testing.T) {
	...
	if e.Type != "movie" || e.APA7.Title == "" {
		t.Fatalf("invalid entry: %+v", e)
	}
	if e.APA7.URL == "" {
		t.Fatalf("expected url populated")
	}
}
...
func TestFetchMovie_TMDbFallback(t *testing.T) {
	...
	e, err := FetchMovie(context.Background(), "Film", "2020-01-02")
	if err != nil {
		t.Fatalf("TMDb fallback: %v", err)
	}
	if e.APA7.Title == "" || len(e.APA7.Authors) == 0 {
		t.Fatalf("expected director from credits: %+v", e)
	}
}
