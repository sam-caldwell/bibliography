...
func fetchFromOMDb(ctx context.Context, title string, date string, apiKey string) (schema.Entry, error) {
    if apiKey == "" { return schema.Entry{}, fmt.Errorf("omdb: missing api key") }
    ...
    if strings.ToLower(out.Response) != "true" {
        if strings.TrimSpace(out.Error) != "" { return schema.Entry{}, fmt.Errorf("omdb: %s", out.Error) }
...
func buildOMDbRequest(ctx context.Context, title, date, apiKey string) *http.Request {
    ...
    q.Set("t", title)
    q.Set("type", "movie")
    if y := dates.YearFromDate(date); y > 0 { q.Set("y", fmt.Sprintf("%d", y)) }
    q.Set("apikey", apiKey)
    u.RawQuery = q.Encode()
...
func decodeOMDb(resp *http.Response) (omdbResp, error) {
    var out omdbResp
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return omdbResp{}, err }
    return out, nil
}
...
func mapOMDbToEntry(out omdbResp, reqTitle, reqDate string) schema.Entry {
    ...
    if e.APA7.Title == "" { e.APA7.Title = reqTitle }
    e.APA7.Date = parseReleasedDate(out.Released)
    if e.APA7.Date == "" { e.APA7.Date = strings.TrimSpace(reqDate) }
    if y := dates.YearFromDate(e.APA7.Date); y > 0 { y2 := y; e.APA7.Year = &y2 }
    ...
    if e.Annotation.Summary == "" { e.Annotation.Summary = fmt.Sprintf("Film: %s.", e.APA7.Title) }
...
func parseReleasedDate(s string) string {
    ...
    if s == "" { return "" }
    if t, err := time.Parse("02 Jan 2006", s); err == nil { return t.Format("2006-01-02") }
    return ""
}

func primaryLink(out omdbResp) string {
    if u := strings.TrimSpace(out.Website); u != "" { return u }
    if id := strings.TrimSpace(out.ImdbID); id != "" { return "https://www.imdb.com/title/" + id }
    return ""
}
...
func fetchFromTMDb(ctx context.Context, title string, date string, apiKey string) (schema.Entry, error) {
    if apiKey == "" { return schema.Entry{}, fmt.Errorf("tmdb: missing api key") }
    req := buildTMDbSearchRequest(ctx, title, date, apiKey)
    resp, err := client.Do(req)
    if err != nil { return schema.Entry{}, err }
    defer resp.Body.Close()
    if resp.StatusCode < 200 || resp.StatusCode >= 300 { return schema.Entry{}, fmt.Errorf("tmdb: http %d", resp.StatusCode) }
    out, err := decodeTMDbSearch(resp)
    if err != nil { return schema.Entry{}, err }
    if len(out.Results) == 0 { return schema.Entry{}, fmt.Errorf("tmdb: no results") }
...
type tmdbSearchResp struct { Results []tmdbResult `json:"results"` }
type tmdbResult struct { ID int `json:"id"`; Title string `json:"title"`; Overview string `json:"overview"`; ReleaseDate string `json:"release_date"` }
...
func buildTMDbSearchRequest(ctx context.Context, title, date, apiKey string) *http.Request {
    ...
    q.Set("query", title)
    if y := dates.YearFromDate(date); y > 0 { q.Set("year", fmt.Sprintf("%d", y)) }
    u.RawQuery = q.Encode()
    req, _ := http.NewRequestWithContext(ctx, http.MethodGet, u.String(), nil)
    httpx.SetUA(req)
    req.Header.Set("Accept", "application/json")
...
func decodeTMDbSearch(resp *http.Response) (tmdbSearchResp, error) {
    ...
    if err := json.NewDecoder(resp.Body).Decode(&out); err != nil { return tmdbSearchResp{}, err }
    return out, nil
}
...
func mapTMDbToEntry(ctx context.Context, r tmdbResult, title, date, apiKey string) schema.Entry {
    ...
    e.Type = "movie"
    e.ID = schema.NewID()
    if strings.TrimSpace(r.Title) != "" { e.APA7.Title = r.Title } else { e.APA7.Title = title }
    e.APA7.Date = strings.TrimSpace(r.ReleaseDate)
    if e.APA7.Date == "" { e.APA7.Date = strings.TrimSpace(date) }
    if y := dates.YearFromDate(e.APA7.Date); y > 0 { y2 := y; e.APA7.Year = &y2 }
...
func fetchTMDbDirectors(ctx context.Context, movieID int, apiKey string) []string {
    ...
    defer resp.Body.Close()
    var c struct{ Crew []struct{ Job, Name string } `json:"crew"` }
    if json.NewDecoder(resp.Body).Decode(&c) != nil { return nil }
    var namesOut []string
    for _, m := range c.Crew {
        if strings.EqualFold(strings.TrimSpace(m.Job), "Director") { namesOut = append(namesOut, m.Name) }
    }
    return namesOut
}
