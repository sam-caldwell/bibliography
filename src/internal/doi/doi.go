...
type CSLAuthor struct { Given string `json:"given"`; Family string `json:"family"` }

type CSLIssued struct { DateParts [][]int `json:"date-parts"` }
...
func mapCSLToEntry(c CSL) schema.Entry {
    ...
    if y, md := yearAndDate(c.Issued); y > 0 { e.APA7.Year = &y; if md != "" { e.APA7.Date = md } }
    ...
    for _, a := range c.Author {
        if strings.TrimSpace(a.Family) == "" { continue }
...
func yearAndDate(i CSLIssued) (int, string) {
    if len(i.DateParts) == 0 || len(i.DateParts[0]) == 0 { return 0, "" }
    dp := i.DateParts[0]
    y := dp[0]
