...
type seqDoer struct{ bodies []string; i int }

func (s *seqDoer) Do(req *http.Request) (*http.Response, error) {
    if s.i >= len(s.bodies) { s.bodies = append(s.bodies, s.bodies[len(s.bodies)-1]) }
    b := s.bodies[s.i]
    s.i++
    if b == "" { b = `{"choices":[{"message":{"content":"[]"}}]}` }
...
func TestGenerateSongFromTitleArtistDate(t *testing.T) {
    ...
    if err != nil { t.Fatalf("GenerateSong: %v", err) }
    if e.Type != "song" || e.APA7.Title == "" || len(e.Annotation.Keywords) == 0 { t.Fatalf("bad song entry: %+v", e) }
}
