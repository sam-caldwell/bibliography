...
func TestMergeSortDedupKeywords(t *testing.T) {
    ...
    for _, w := range []string{"go","yaml","cli","tool"} {
        if !strings.Contains(got, w) { t.Fatalf("missing keyword %q in %q", w, got) }
    }
}
...
func TestURLAccessible(t *testing.T) {
    ...
    defer srv.Close()
    if !urlAccessible(context.Background(), srv.URL) { t.Fatalf("expected urlAccessible true for test server") }
}
