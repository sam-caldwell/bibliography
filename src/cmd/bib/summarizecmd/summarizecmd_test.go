...
func TestGatherCitationPaths_FiltersYAML(t *testing.T) {
	...
	if len(paths) != 2 {
		t.Fatalf("want 2 yaml paths, got %d: %+v", len(paths), paths)
	}
}
...
func TestProcessSummary_SkipOnUnreachableAndSummarizeError(t *testing.T) {
	...
	if err != nil || changed {
		t.Fatalf("expected skip: changed=%v err=%v", changed, err)
	}
	if !strings.Contains(errBuf.String(), "skip ") {
		t.Fatalf("expected skip message, got: %q", errBuf.String())
	}
	...
	if err != nil || changed {
		t.Fatalf("expected skip on summarize error: changed=%v err=%v", changed, err)
	}
}
