...
func TestIndexCommand_WritesIndexesAndCommits(t *testing.T) {
    ...
    // stub commit
    called := 0; var gotPaths []string
    commit := func(paths []string, message string) error { called++; gotPaths = paths; return nil }
    cmd := New(commit)
    var buf bytes.Buffer
    cmd.SetOut(&buf)
    if err := cmd.RunE(cmd, []string{}); err != nil { t.Fatalf("index run: %v", err) }
    if called == 0 { t.Fatalf("expected commit to be called") }
    if len(gotPaths) != 1 || !strings.Contains(gotPaths[0], store.MetadataDir) { t.Fatalf("unexpected commit paths: %+v", gotPaths) }
    // verify files
    if _, err := os.Stat(filepath.Join("data", "metadata", "keywords.json")); err != nil { t.Fatalf("keywords.json not written") }
}
