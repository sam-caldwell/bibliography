...
// newSummarizeCmd creates the "summarize" command to fill summaries/keywords via OpenAI.
...
func loadEntryIfSummarizable(path string) (schema.Entry, bool, error) {
    ...
    if err != nil { return schema.Entry{}, false, err }
    var e schema.Entry
    if err := yaml.Unmarshal(b, &e); err != nil { return schema.Entry{}, false, nil }
...
// urlAccessible performs a quick HEAD/GET to determine if a URL is reachable (2xx/3xx).
