...
func TestSetYAMLPathValue_OverwriteValue(t *testing.T) {
	root := &yaml.Node{Kind: yaml.MappingNode, Tag: "!!map"}
	if err := setYAMLPathValue(root, "apa7.title", "Old"); err != nil {
		t.Fatal(err)
	}
	if err := setYAMLPathValue(root, "apa7.title", "New"); err != nil {
		t.Fatal(err)
	}
	apa7 := findMapValue(root, "apa7")
	title := findMapValue(apa7, "title")
...
func TestSetYAMLPathValue_TypeChangeOnLeaf(t *testing.T) {
	...
	if err := setYAMLPathValue(root, "apa7.title", "X"); err != nil {
		t.Fatal(err)
	}
	// Replace apa7 mapping with a scalar
	if err := setYAMLPathValue(root, "apa7", "scalar"); err != nil {
		t.Fatal(err)
	}
	apa7 := findMapValue(root, "apa7")
...
func TestSetYAMLPathValue_FallbackToStringOnInvalidYAML(t *testing.T) {
	root := &yaml.Node{Kind: yaml.MappingNode, Tag: "!!map"}
	raw := ":\n" // invalid YAML
	if err := setYAMLPathValue(root, "note", raw); err != nil {
		t.Fatal(err)
	}
