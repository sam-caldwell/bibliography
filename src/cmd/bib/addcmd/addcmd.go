...
func (b Builder) Article() *cobra.Command {
    ...
    c := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if len(h) == 0 { return manualAdd(cmd, b.Commit, "article", parseKeywordsCSV(artKeywords)) }
...
func (b Builder) Patent() *cobra.Command {
    ...
    c := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            if len(h) == 0 { return manualAdd(cmd, b.Commit, "patent", parseKeywordsCSV(patKeywords)) }
...
func (b Builder) writeCommitPrint(cmd *cobra.Command, e schema.Entry) error {
    ...
    if err != nil { return err }
    if err := b.Commit([]string{path}, fmt.Sprintf(msgAddCitation, e.ID)); err != nil { return err }
    _, err = fmt.Fprintf(cmd.OutOrStdout(), msgWrote, path)
    return err
}

func applyKeywordsOverride(e *schema.Entry, kwCSV string) {
    if ks := parseKeywordsCSV(kwCSV); len(ks) > 0 { e.Annotation.Keywords = ks }
}

func ensureTypeKeyword(e *schema.Entry, typ string) {
    if e == nil { return }
    if len(e.Annotation.Keywords) == 0 { e.Annotation.Keywords = []string{typ} }
}
...
func hintsBook(name, author, isbn string) map[string]string {
    m := map[string]string{}
    if strings.TrimSpace(name) != "" { m["title"] = name }
    if strings.TrimSpace(author) != "" { m["author"] = author }
    if strings.TrimSpace(isbn) != "" { m["isbn"] = isbn }
    return m
}

func hintsMovie(title, date string) map[string]string {
    m := map[string]string{"title": title}
    if strings.TrimSpace(date) != "" { m["date"] = date }
    return m
}

func hintsSong(title, artist, date string) map[string]string {
    m := map[string]string{"title": title}
    if strings.TrimSpace(artist) != "" { m["author"] = artist }
    if strings.TrimSpace(date) != "" { m["date"] = date }
    return m
}
...
func hintsArticle(title, author, journal, date string) map[string]string {
    m := map[string]string{}
    if strings.TrimSpace(title) != "" { m["title"] = title }
    if strings.TrimSpace(author) != "" { m["author"] = author }
    if strings.TrimSpace(journal) != "" { m["journal"] = journal }
    if strings.TrimSpace(date) != "" { m["date"] = date }
    return m
}
...
func hintsPatent(urlStr, title, inventor, assignee, date string) map[string]string {
    m := map[string]string{}
    if strings.TrimSpace(title) != "" { m["title"] = title }
    if strings.TrimSpace(inventor) != "" { m["author"] = inventor }
    if strings.TrimSpace(assignee) != "" { m["publisher"] = assignee }
    if strings.TrimSpace(date) != "" { m["date"] = date }
    if strings.TrimSpace(urlStr) != "" { m["url"] = strings.TrimSpace(urlStr) }
    return m
}
...
func parseKeywordsCSV(s string) []string {
    ...
    for _, p := range parts {
        ...
        if p == "" || seen[strings.ToLower(p)] { continue }
...
func doAddWithKeywords(ctx context.Context, commit CommitFunc, typ string, hints map[string]string, extraKeywords []string) error {
    ...
    path, err := store.WriteEntry(e)
    if err != nil { return err }
    if err = commit([]string{path}, fmt.Sprintf(msgAddCitation, e.ID)); err != nil { return err }
    if _, err = fmt.Fprintf(os.Stdout, msgWrote, path); err != nil { return err }
    return nil
}
...
func deriveTitle(typ string, hints map[string]string) (string, error) {
    ...
    if (typ == "website" || typ == "patent") && title == "" {
        if u := strings.TrimSpace(hints["url"]); u != "" {
            if pu, err := url.Parse(u); err == nil && pu.Host != "" { title = pu.Host } else { title = u }
        }
    }
    if title == "" { return "", fmt.Errorf("title is required for %s adds without external metadata", typ) }
    return title, nil
}

func applyJournal(e *schema.Entry, hints map[string]string) {
    if v := strings.TrimSpace(hints["journal"]); v != "" { e.APA7.Journal, e.APA7.ContainerTitle = v, v }
}

func applyDate(e *schema.Entry, hints map[string]string) {
    if v := strings.TrimSpace(hints["date"]); v != "" {
        e.APA7.Date = v
        if y := dates.YearFromDate(v); y > 0 { y2 := y; e.APA7.Year = &y2 }
    }
}

func applyURL(e *schema.Entry, hints map[string]string) {
    if v := strings.TrimSpace(hints["url"]); v != "" { e.APA7.URL = v }
}

func applyAuthorHint(e *schema.Entry, hints map[string]string) {
    if v := strings.TrimSpace(hints["author"]); v != "" {
        fam, giv := parseAuthor(v)
        if fam != "" { e.APA7.Authors = append(e.APA7.Authors, schema.Author{Family: fam, Given: giv}) }
    }
}

func applyIDs(e *schema.Entry, hints map[string]string) {
    if v := strings.TrimSpace(hints["isbn"]); v != "" { e.APA7.ISBN = v }
    if v := strings.TrimSpace(hints["doi"]); v != "" { e.APA7.DOI = v; if e.APA7.URL == "" { e.APA7.URL = "https://doi.org/" + v } }
}

func applyDefaults(e *schema.Entry, typ string, extraKeywords []string) {
    e.ID = schema.NewID()
    if len(extraKeywords) > 0 { e.Annotation.Keywords = extraKeywords }
    if len(e.Annotation.Keywords) == 0 { e.Annotation.Keywords = []string{typ} }
}

func applyManualSummary(e *schema.Entry) {
    if strings.TrimSpace(e.APA7.Title) == "" { return }
...
func collectManualFields(cmd *cobra.Command, typ string, extraKeywords []string) (manualFields, error) {
    ...
    if mf.title == "" { return manualFields{}, fmt.Errorf("title is required") }
    ...
    case "rfc":
        ...
        if mf.publisher == "" { mf.publisher = "IETF" }
    }
    mf.summary = strings.TrimSpace(prompt(cmd, in, out, "Summary (required): "))
    if mf.summary == "" { mf.summary = fmt.Sprintf("Bibliographic record for %s (manually entered).", mf.title) }
    mf.keywords = parseKeywordsCSV(strings.TrimSpace(prompt(cmd, in, out, "Keywords (comma-separated; optional): ")))
    if len(mf.keywords) == 0 { mf.keywords = []string{typ} }
    if len(extraKeywords) > 0 { mf.keywords = append(mf.keywords, extraKeywords...) }
    return mf, nil
}
...
func buildManualEntry(typ string, mf manualFields) (schema.Entry, error) {
    ...
    if y := dates.YearFromDate(mf.date); y > 0 { y2 := y; e.APA7.Year = &y2 }
    e.APA7.Date = mf.date
    e.APA7.URL = mf.url
    e.APA7.DOI = mf.doi
    e.APA7.ISBN = mf.isbn
    if strings.TrimSpace(e.APA7.URL) != "" { e.APA7.Accessed = dates.NowISO() }
...
func prompt(cmd *cobra.Command, in io.Reader, out io.Writer, q string) string {
    if _, err := fmt.Fprint(out, q); err != nil { return "" }
...
func getMovieEntry(ctx context.Context, title, date string) (schema.Entry, bool) {
    if e, err := moviefetch.FetchMovie(ctx, title, date); err == nil { return e, true }
    if e, err := summarize.GenerateMovieFromTitleAndDate(ctx, title, date); err == nil { return e, true }
    return schema.Entry{}, false
}

func getSongEntry(ctx context.Context, title, artist, date string) (schema.Entry, bool) {
    if e, err := songfetch.FetchSong(ctx, title, artist, date); err == nil { return e, true }
    if e, err := summarize.GenerateSongFromTitleArtistDate(ctx, title, artist, date); err == nil { return e, true }
    return schema.Entry{}, false
}
