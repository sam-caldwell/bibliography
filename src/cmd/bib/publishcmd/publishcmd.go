...
// New returns the publish command to generate HTML pages for all entries.
    ...
    cmd := &cobra.Command{
        ...
        RunE: func(cmd *cobra.Command, args []string) error {
            ...
            for _, e := range entries {
                ...
                if err := writeEntryPage(e, seg); err != nil { return err }
            }
            if err := writeIndex(byCat); err != nil { return err }
...
var entryTmpl = template.Must(template.New("entry").Funcs(template.FuncMap{"join": strings.Join,}).Parse(`<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
type entryPageModel struct { schema.Entry; Canonical string }
...
type indexCategory struct { Name string; Items []indexItem }
type indexItem struct { Title string; Href string }
type indexModel struct { Categories []indexCategory }

func writeIndex(byCat map[string][]schema.Entry) error {
    var cats []string
    for c := range byCat { cats = append(cats, c) }
    sort.Strings(cats)
    ...
    for _, c := range cats {
        ...
        var items []indexItem
        for _, e := range es { items = append(items, indexItem{Title: e.APA7.Title, Href: filepath.ToSlash(filepath.Join(c, e.ID+".html"))}) }
        model.Categories = append(model.Categories, indexCategory{Name: c, Items: items})
    }
    f, err := os.Create(filepath.Join("docs", "index.html")); if err != nil { return err }
    defer f.Close()
