...
func TestAPACitation_VariousTypes(t *testing.T) {
	...
	if c := APACitation(be); !strings.Contains(c, "P") {
	...
	if c := APACitation(we); !strings.Contains(c, "Site") {
	...
	if c := APACitation(me); !strings.Contains(c, "[Film]") {
	...
	if c := APACitation(ve); !strings.Contains(c, "[Video]") {
		t.Fatalf("video cite missing [Video]: %q", c)
	}
	// Song
	...
	if c := APACitation(pe); !strings.Contains(c, "USPTO") {
	...
	if c := APACitation(re); !strings.Contains(c, "RFC") {
