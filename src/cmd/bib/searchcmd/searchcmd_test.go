...
func TestParseExprPredicates(t *testing.T) {
    ...
    if err != nil { t.Fatalf("parseExpr: %v", err) }
    ...
    hit := true; score := 0
    for _, p := range preds { h, s := p(e); if !h { hit = false; break }; score += s }
    if !hit { t.Fatalf("expected predicates to match entry; score=%d", score) }
    // Negative path
    e.APA7.Title = "An Intro to Rust"
    e.Annotation.Keywords = []string{"rust"}
    hit2 := true
    for _, p := range preds { h, _ := p(e); if !h { hit2 = false; break } }
    if hit2 { t.Fatalf("expected predicates to fail with changed title") }
}
