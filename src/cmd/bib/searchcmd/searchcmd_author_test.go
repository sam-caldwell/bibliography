...
func TestAuthorEqualsWildcard(t *testing.T) {
    ...
    if err != nil || !ok { t.Fatalf("compile author wildcard: ok=%v err=%v", ok, err) }
    hit, score := p(e)
    if !hit || score == 0 { t.Fatalf("expected hit with score, got hit=%v score=%d", hit, score) }
    ...
    if buf.Len() == 0 { t.Fatalf("expected table output") }
}
